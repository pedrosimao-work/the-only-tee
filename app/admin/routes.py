...
import logging  # Import logging so admin route failures can be recorded
...
from app.services.printify_catalog import PrintifyProductSelectionError, apply_printify_product_selection, list_available_printify_products  # Import Printify product picker helpers
...
logger = logging.getLogger(__name__)  # Create a module logger for admin route failures
...
def load_printify_product_choices(current_drop=None):  # Define a helper that loads selectable Printify products for the admin picker
    try:  # Try to load available Printify products
        return list_available_printify_products(current_drop=current_drop), None  # Return product choices and no error
    except (PrintifyConfigError, PrintifyAPIError) as error:  # Catch Printify config or API errors
        logger.exception("Printify product picker failed to load choices.")  # Log the technical picker loading failure
        return [], str(error)  # Return no choices and a safe error message
...
def create_drop():  # Define the function that handles creating new drops
    ...
    printify_products, printify_error = load_printify_product_choices()  # Load Printify products not already connected to local drops
    ...
    if form.validate_on_submit():  # Check if the form was submitted and passed validation
        ...
        if existing_drop:  # Check if another drop already uses this drop number
            ...
            return render_template("admin/create_drop.html", form=form, printify_products=printify_products, printify_error=printify_error)  # Re-render the create-drop form
        ...
        drop = Drop(  # Create a new Drop object from the form data
            ...
            name=form.name.data.strip(),  # Store the manually written drop name
            description=form.description.data.strip(),  # Store the manually written drop description
        ...
        try:  # Try to save the new drop, apply Printify selection, sync mockups, and sync Stripe
            db.session.flush()  # Assign a local database ID before creating external metadata
            selected_printify_product_id = clean_optional_text(form.printify_product_id.data)  # Read the selected Printify product ID from the picker
            apply_printify_product_selection(drop, selected_printify_product_id)  # Save Printify product ID and variant IDs automatically

            if selected_printify_product_id:  # Check if a Printify product was selected
                sync_drop_with_printify(drop)  # Sync mockups, images, available sizes, and size-variant mapping

            ensure_stripe_product_and_price_for_drop(drop)  # Automatically create the Stripe Product and USD Stripe Price for this drop
            db.session.commit()  # Save the local drop and external IDs permanently
        except (PrintifyConfigError, PrintifyAPIError, PrintifyProductSelectionError, StripeProductSyncError) as error:  # Catch Printify picker/sync and Stripe sync errors
            logger.exception("Admin drop creation failed during external sync for Drop #%s.", form.drop_number.data)  # Log the technical create-drop sync failure
            db.session.rollback()  # Undo the local drop creation because external sync failed
            flash(str(error), "danger")  # Show the sync error to the admin user
            return render_template("admin/create_drop.html", form=form, printify_products=printify_products, printify_error=printify_error)  # Re-render the create-drop form
        except IntegrityError:  # Catch database-level duplicate or constraint errors
            logger.exception("Admin drop creation failed because of a database constraint for Drop #%s.", form.drop_number.data)  # Log the technical database failure
        ...
        flash(f"Drop #{drop.drop_number} was created and synced.", "success")  # Show a success message after creating and syncing the drop
        return redirect(url_for("admin.drops"))  # Redirect the admin user to the drop list

    return render_template("admin/create_drop.html", form=form, printify_products=printify_products, printify_error=printify_error)  # Render the create-drop form for GET requests or invalid submissions
...
def edit_drop(drop_id):  # Define the function that handles editing existing drops
    ...
    printify_products, printify_error = load_printify_product_choices(current_drop=drop)  # Load unassigned Printify products plus the current drop's product

    if not form.is_submitted():  # Check if this is the initial GET request
        form.printify_product_id.data = drop.printify_product_id or ""  # Pre-select the current Printify product in the picker
    ...
    if form.validate_on_submit():  # Check if the edit form was submitted and passed validation
        ...
        if existing_drop:  # Check if another drop already uses this drop number
            ...
            return render_template("admin/edit_drop.html", form=form, drop=drop, printify_products=printify_products, printify_error=printify_error)  # Re-render the edit-drop form
        ...
        drop.name = form.name.data.strip()  # Update the manually written drop name
        drop.description = form.description.data.strip()  # Update the manually written drop description
        ...
        try:  # Try to save the edited drop, apply Printify selection, sync mockups, and sync Stripe
            selected_printify_product_id = clean_optional_text(form.printify_product_id.data)  # Read the selected Printify product ID from the picker
            apply_printify_product_selection(drop, selected_printify_product_id)  # Save Printify product ID and variant IDs automatically
            ...
            db.session.commit()  # Save the updated drop and external IDs permanently
        except (PrintifyConfigError, PrintifyAPIError, PrintifyProductSelectionError, StripeProductSyncError) as error:  # Catch Printify picker/sync and Stripe sync errors
            logger.exception("Admin drop update failed during external sync for Drop #%s.", drop.drop_number)  # Log the technical edit-drop sync failure
            db.session.rollback()  # Undo the edited drop changes because external sync failed
            flash(str(error), "danger")  # Show the sync error to the admin user
            return render_template("admin/edit_drop.html", form=form, drop=drop, printify_products=printify_products, printify_error=printify_error)  # Re-render the edit-drop form
        except IntegrityError:  # Catch database-level duplicate or constraint errors
            logger.exception("Admin drop update failed because of a database constraint for Drop #%s.", drop.drop_number)  # Log the technical database failure
        ...
        flash(f"Drop #{drop.drop_number} was updated and synced.", "success")  # Show a success message after updating and syncing the drop
        return redirect(url_for("admin.drops"))  # Redirect the admin user back to the drop list

    return render_template("admin/edit_drop.html", form=form, drop=drop, printify_products=printify_products, printify_error=printify_error)  # Render the edit-drop form for GET requests or invalid submissions
...
def delete_drop(drop_id):  # Define the function that deletes safe local drops
    ...
    try:  # Try to clean the Stripe catalog before deleting the local drop
        ...
        logger.exception("Admin drop deletion failed during Stripe cleanup for Drop #%s.", drop.drop_number)  # Log the technical delete-drop cleanup failure
...
def sync_printify_drop(drop_id):  # Define the function that syncs one drop with Printify
    ...
    try:  # Start a protected block for Printify sync
        ...
        db.session.commit()  # Save the manual Printify sync changes because the service no longer commits internally
    except (PrintifyConfigError, PrintifyAPIError) as error:  # Catch Printify configuration and API errors
        logger.exception("Manual Printify sync failed for Drop #%s.", drop.drop_number)  # Log the technical manual Printify sync failure
        db.session.rollback()  # Roll back any partial Printify sync changes
...
def fulfill_printify_order(order_id):  # Define the admin action that submits a paid order to Printify
    ...
    try:  # Start a protected block for Printify fulfillment
        ...
        logger.exception("Manual Printify fulfillment failed for Order #%s.", order.id)  # Log the technical manual fulfillment failure
