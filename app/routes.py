...
def home():  # Define the function that runs when a user visits the homepage
    active_drop = Drop.query.filter_by(status="active").first()  # Query the database for the first drop with active status
...
def archive():  # Define the function that runs when a user visits the archive page
    archived_drops = Drop.query.filter_by(status="archived").order_by(Drop.drop_number.asc()).all()  # Query archived drops ordered by drop number
